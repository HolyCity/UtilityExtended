...
*/
CUtilityExtended::CUtilityExtended()
{
...
*/
CUtilityExtended::~CUtilityExtended(void)
{
	if (m_pBufferL)
		delete[] m_pBufferL;
	if (m_pBufferR)
...
*/
bool __stdcall CUtilityExtended::initialize()
{
	// Add your code here
...
void CUtilityExtended::cookVariables()
{
	m_fGain = powf(10.0, m_fGain_dB / 20.0);
	//Find the read locations for the DDL, wrap if necessary
	m_nReadIndexL = m_nWriteIndexL - m_fDelayLeftSamples;
	m_nReadIndexR = m_nWriteIndexR - m_fDelayRightSamples;
	if (m_nReadIndexL < 0)
		m_nReadIndexL += m_nBufferSize;
	if (m_nReadIndexR < 0)
		m_nReadIndexR += m_nBufferSize;

}

void CUtilityExtended::resetDelay()
{
	if (m_pBufferL)
		memset(m_pBufferL, 0, m_nBufferSize*sizeof(float));
	m_nWriteIndexL = 0;
	m_nWriteIndexR = 0;
...
*/
bool __stdcall CUtilityExtended::prepareForPlay()
{
	// Add your code here:

	//Initialize the filters
	leftFilter.setSampleRate(m_nSampleRate);
	rightFilter.setSampleRate(m_nSampleRate);
	leftAP.setSamplerate(m_nSampleRate);
	rightAP.setSamplerate(m_nSampleRate);
	leftFilter.setCutoff(m_fLinkwitzRileyCutoff);
	rightFilter.setCutoff(m_fLinkwitzRileyCutoff);
	...
	if (m_pBufferR)
		delete[] m_pBufferR;
	m_pBufferL = new float[m_nBufferSize];
	m_pBufferR = new float[m_nBufferSize];
	resetDelay();
	//Cook variables
	cookVariables();
	return true;
...
*/
bool __stdcall CUtilityExtended::processAudioFrame(float* pInputBuffer, float* pOutputBuffer, UINT uNumInputChannels, UINT uNumOutputChannels)
{
	...
	if (m_uLinkwitzRileyMode == 1)
	{
		left = leftFilter.doFilter(pInputBuffer[0]);
		right = rightFilter.doFilter(pInputBuffer[1]);
	}
	else if (m_uLinkwitzRileyMode == 2)
	{
		//For the second order Linkwitz-Riley second order implementation to work, one output must be inverted!
		left = -1.0*leftFilter.doFilter(pInputBuffer[0]);
		right = -1.0*rightFilter.doFilter(pInputBuffer[1]);
	}
	//if filter is off, left/right = input
	else
	{
		left = pInputBuffer[0];
		right = pInputBuffer[1];
	}
	// All-pass filter implementation
	if (m_uAllpassBypass == 1)
	{
		float tempLeft = leftAP.doFilter(left);
		float tempRight = rightAP.doFilter(right);
		left = tempLeft;
		right = tempRight;
	}
	float leftSide;
	float rightSide;
	//Output mode switch matrix
	switch (m_uOutputMode)
	{
		case Stereo:
		{
			leftOut = left;
		...
		case Mono:
		{
			leftOut = 0.5*(left + right);
			rightOut = 0.5*(left + right);
			break;
		}
		case Mid:
		{
	...
	//phase inversion matrix
	if (m_uPhaseInvert == SWITCH_ON)
	{
		leftOut = -1.0 * leftOut;
		rightOut = -1.0 * rightOut;
	}

	/*//read from buffers
	//Left delay
	float leftDelayed = m_pBufferL[m_nReadIndexL];
	if (m_nReadIndexL == m_nWriteIndexL)
		leftDelayed = leftOut;
	float rightDelayed = m_pBufferR[m_nReadIndexR];
	if (m_nReadIndexR == m_nWriteIndexR)
		rightDelayed = rightOut;

	//write to buffers
	m_pBufferL[m_nWriteIndexL] = leftOut;
	m_pBufferR[m_nWriteIndexR] = rightOut;

	//increment indices and wrap if necessary
	m_nReadIndexL++;
	m_nReadIndexR++;
	m_nWriteIndexL++;
	m_nWriteIndexR++;
	if (m_nWriteIndexL >= m_nBufferSize)
		m_nWriteIndexL = 0;
	if (m_nWriteIndexR >= m_nBufferSize)
		m_nWriteIndexR = 0;

	leftOut = leftDelayed;
	rightOut = rightDelayed;*/

	pOutputBuffer[0] = m_fGain*leftOut;
	pOutputBuffer[1] = m_fGain*rightOut;
...
-----------------------------------------------
	m_fGain_dB                        0
	m_fLinkwitzRileyCutoff            1
	m_fCornerFreq                     2
	m_fDelayLeftSamples               3
	m_fDelayRightSamples              4
	m_fWidth                          5
	m_uOutputMode                     41
	m_uLinkwitzRileyMode              42
	m_uPhaseInvert                    45
	m_uAllpassBypass                  46
...
//
bool __stdcall CUtilityExtended::userInterfaceChange(int nControlIndex)
{
	cookVariables();
	...
	switch(nControlIndex)
	{
		...
		case 42:
		{
			switch (m_uLinkwitzRileyMode)
			{
				case Off:
					break;
				case HP:
				{
					leftFilter.setType(1);
					rightFilter.setType(1);
					break;
				}
				case LP:
				{
					leftFilter.setType(0);
					rightFilter.setType(0);
					break;
				}
			}
		}
...
*/
bool __stdcall CUtilityExtended::joystickControlChange(float fControlA, float fControlB, float fControlC, float fControlD, float fACMix, float fBDMix)
{
	// add your code here
...
*/
bool __stdcall CUtilityExtended::processRackAFXAudioBuffer(float* pInputBuffer, float* pOutputBuffer,
...
*/
bool __stdcall CUtilityExtended::processVSTAudioBuffer(float** inBuffer, float** outBuffer, UINT uNumChannels, int inFramesToProcess)
{
...
bool __stdcall CUtilityExtended::midiNoteOn(UINT uChannel, UINT uMIDINote, UINT uVelocity)
{
	return true;
}

bool __stdcall CUtilityExtended::midiNoteOff(UINT uChannel, UINT uMIDINote, UINT uVelocity, bool bAllNotesOff)
{
	return true;
}

// uModValue = 0->127
bool __stdcall CUtilityExtended::midiModWheel(UINT uChannel, UINT uModValue)
{
	return true;
}
...
bool __stdcall CUtilityExtended::midiPitchBend(UINT uChannel, int nActualPitchBendValue, float fNormalizedPitchBendValue)
{
	return true;
}
...
*/
bool __stdcall CUtilityExtended::midiClock()
{
...
bool __stdcall CUtilityExtended::midiMessage(unsigned char cChannel, unsigned char cStatus, unsigned char
...
bool __stdcall CUtilityExtended::initUI()
{
	// ADDED BY RACKAFX -- DO NOT EDIT THIS CODE!!! ------------------------------ //
...
// **--0xDEA7--**
	...
	ui0->fUserDisplayDataHiLimit = 12.000000;
	...
	ui2->fInitUserFloatValue = 0.100000;
	...
	ui2->dPresetData[0] = 0.100000;ui2->dPresetData[1] = 0.000000;ui2->dPresetData[2] = 0.000000;ui2->dPresetData[3] = 0.000000;ui2->dPresetData[4] = 0.000000;ui2->dPresetData[5] = 0.000000;ui2->dPresetData[6] = 0.000000;ui2->dPresetData[7] = 0.000000;ui2->dPresetData[8] = 0.000000;ui2->dPresetData[9] = 0.000000;ui2->dPresetData[10] = 0.000000;ui2->dPresetData[11] = 0.000000;ui2->dPresetData[12] = 0.000000;ui2->dPresetData[13] = 0.000000;ui2->dPresetData[14] = 0.000000;ui2->dPresetData[15] = 0.000000;
	ui2->cControlName = "Corner";
	...
	ui3->fUserDisplayDataHiLimit = 1024.000000;
	...
	ui3->m_pUserCookedIntData = &m_fDelayLeftSamples;
	ui3->m_pUserCookedFloatData = NULL;
	ui3->m_pUserCookedDoubleData = NULL;
	ui3->m_pUserCookedUINTData = NULL;
	ui3->cControlUnits = "Samples                                                         ";
	ui3->cVariableName = "m_fDelayLeftSamples";
	...
	ui4->fUserDisplayDataHiLimit = 1024.000000;
	ui4->uUserDataType = intData;
	ui4->fInitUserIntValue = 0.000000;
	ui4->fInitUserFloatValue = 0;
	ui4->fInitUserDoubleValue = 0;
	ui4->fInitUserUINTValue = 0;
	ui4->m_pUserCookedIntData = &m_fDelayRightSamples;
	ui4->m_pUserCookedFloatData = NULL;
	ui4->m_pUserCookedDoubleData = NULL;
	ui4->m_pUserCookedUINTData = NULL;
	ui4->cControlUnits = "Samples                                                         ";
	ui4->cVariableName = "m_fDelayRightSamples";
	...
	ui6->fUserDisplayDataHiLimit = 5.000000;
	...
	ui6->m_pUserCookedUINTData = &m_uOutputMode;
	ui6->cControlUnits = "";
	ui6->cVariableName = "m_uOutputMode";
	ui6->cEnumeratedList = "Stereo,Mono,Mid,Side,Left,Right";
	...
	ui7->m_pUserCookedUINTData = &m_uLinkwitzRileyMode;
	ui7->cControlUnits = "";
	ui7->cVariableName = "m_uLinkwitzRileyMode";
	ui7->cEnumeratedList = "Off,LP,HP";
	...
	ui7->cControlName = "Linkwitz-Riley Mode";
	...
	ui8->uControlType = FILTER_CONTROL_RADIO_SWITCH_VARIABLE;
	ui8->uControlId = 45;
	...
	ui8->m_pUserCookedUINTData = &m_uPhaseInvert;
	ui8->cControlUnits = "";
	ui8->cVariableName = "m_uPhaseInvert";
	ui8->cEnumeratedList = "SWITCH_OFF,SWITCH_ON";
	...
	ui9->m_pUserCookedUINTData = &m_uAllpassBypass;
	ui9->cControlUnits = "";
	ui9->cVariableName = "m_uAllpassBypass";
	ui9->cEnumeratedList = "SWITCH_OFF,SWITCH_ON";
