...
#include "Allpass.h"
class CUtilityExtended : public CPlugIn
{
public:
	...
	float leftOut;
	float rightOut;
	CLinkwitzRiley leftFilter;
	CLinkwitzRiley rightFilter;
	CAllpass leftAP;
	CAllpass rightAP;
	void cookVariables();
	float m_fGain;
	...
	float m_fGain_dB;
	float m_fLinkwitzRileyCutoff;
	float m_fCornerFreq;
	int m_fDelayLeftSamples;
	int m_fDelayRightSamples;
	float m_fWidth;
	UINT m_uOutputMode;
	enum{Stereo,Mono,Mid,Side,Left,Right};
	UINT m_uLinkwitzRileyMode;
	enum{Off,LP,HP};
	UINT m_uPhaseInvert;
	enum{SWITCH_OFF,SWITCH_ON};
	UINT m_uAllpassBypass;
